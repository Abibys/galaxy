...
let item_op_objects = document.querySelectorAll('.merch-item-descr')
let item_op = ['Тарілка з принтом Марса. Доступна в червоному та білому кольорах',
...
for (let i = 0; i < item_op.length; i+=1 ){
    item_op_objects[i].innerHTML= item_op[i]
}

let item_cina_objects = document.querySelectorAll('.merch-item-price')
let item_cine = ['1000','1000','2500','5000']
for (let i = 0; i <item_cine.length; i += 1){
    item_cina_objects[i].innerHTML = item_cine[i]
}
